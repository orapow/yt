...
// cancelfun:取消回调
//------------------------------------------------
x.dialog.confirm = function (msg, okfun, cancelfun) {
...
// value:默认值
//------------------------------------------------
x.dialog.input = function (title, okfun, type, notnull, value) {
    var inp = "";
...
// okfun:确定回调
// hidetitle:是否显示标题栏
// icon:1:?,2:i,3:!,4:x,5:ok
//------------------------------------------------
x.dialog.alert = function (msg, okfun, hidetitle, icon) {
    var ics = ["qus", "info", "err", "warn", "ok"];
    if (!icon) icon = 2;
    var d = { content: "<i class='win icon " + ics[icon - 1] + "'></i>  " + msg, autofocus: false };
    if (!hidetitle) d.title = "消息提示";
    d.ok = okfun || function () { };
...
// >0、自动隐藏
//------------------------------------------------
x.dialog.loading = function (msg, sec) {
    sec = sec || 0;
    msg = msg || "正在加载中...";
...
x.form = {
    init: function (options) {
        ...
        var def = {
            ...
            postedcallback: null,
            showtipicon: false,
            showmsg: function (i, msg) {
                if (i.attr("editor")) {
                    i.prev("div").css("border-color", "red").attr("title", msg);
                }
                else {
                    i.css("border-color", "#FF0039").css("color", "#FF0039").attr("title", msg);
                }
                var tip = i.data("tip");
                if (tip && this.showtipicon) {
                    tip.content("<i class='icon icon-remove-sign err tip' title='" + msg + "'> " + msg + "</i>");
                    tip.show(i[0]);
                }
                i.next("span[msg]").html(msg).css("color", "#FF0039");
            },
            hidemsg: function (i, hidetip) {
                if (i.attr("editor")) {
                    i.prev("div").css("border-color", "").attr("title", "");
                } else {
                    i.css("color", "#494949").css("border-color", "#ccc").attr("title", "");
                }
                if (i.data("tip") && this.showtipicon) {
                    i.data("tip").close();
                    i.data("tip").remove();
                    i.data("tip", null);
                }
            ...
            valided: function (i) {
                if (i.attr("type") == "hidden") return;
                def.hidemsg(i);
                if (i.attr("editor")) {
                    i.prev("div").css("border-color", "#3fb618").attr("title", i.attr("placeholder") + "输入正确");
                } else {
                    i.attr("title", i.attr("placeholder") + "输入正确");
                }
                var tip = i.data("tip")
                if (tip && this.showtipicon)
                    tip.content("<i class='icon icon-ok-sign ok tip' title='" + i.attr("placeholder") + " 输入正确" + "'> " + i.attr("placeholder") + " 输入正确" + "</i>");
            }
        },
        validator = {
            email: { reg: /^\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/, msg: "Email格式不正确，abc@bb.com" },
            number: { reg: /^\d+$/, msg: "这里只能输入>=0的数字" },
            chinese: { reg: /^[\u0391-\uFFE5]+$/, msg: "这里只能输入汉字" },
            mphone: { reg: /^\d{11}$/, msg: "这里格式不正确，应为手机号码（11位数字）" },
            tel: { reg: /((\d{11})|^((\d{7,8})|(\d{4}|\d{3})-(\d{7,8})|(\d{4}|\d{3})-(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1})|(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1}))$)/, msg: "这里格式不正确，应为电话号码（包括手机号）" },
            ndecimal: { reg: /^\d+([.]\d+)?$/, msg: "这里只能输入小数或整数" },
            date: { reg: /^\d{4}-\d{2}-\d{2}$/, msg: "不是有效的日期格式（格式：1990-01-01）" },
            datetime: { reg: /^\d{4}-\d{2}-\d{2} \d{2}:\d{2}(:\d{2})?$/, msg: "不是有效的日期时间格式，可以不包含秒（格式：1990-01-01 00:00:00）" },
            url: { reg: /([\w\dz\/\-:]+)[.][a-z]{3,4}$/, msg: "不是有效的Url格式，例：http://wwww.abc.com" },
            custom: {},
            character: { reg: /^[a-zA-z0-9\u4E00-\u9FA5\s]+$/, msg: "这里只能输入字母、数字、汉字，不能输入标点符号" },
        };
        def = $.extend(def, options);
        def.form.find(".switch").click(function () { $(this).toggleClass("selected"); })
        def.form.find(".checkbox").click(function () { $(this).toggleClass("btn-primary"); })
        def.form.find(".radio").click(function () {
            var r = $(this);
            $("[name='" + r.attr("name") + "']").removeClass("btn-primary");
            r.addClass("btn-primary");
        })
        def.form.find("[data-src][data-check]").each(function () {
            $(this).append("<i class='icon-caret-down'></i>");
            $(this).click(function () { $(this).focus(); });
        });
        var checkinputs = def.form.find("[data-check]");
        def.form.submit(function (e) {
            if (def.prepost) {
                if (!def.prepost()) return false;
            }
            ...
            if (ispass === 0) {
                var d = {};
                def.form.find("[data-check]").each(function () {
                    var i = $(this);
                    var n = i.attr("name");
                    var val = "";
                    if (i[0].tagName.toLowerCase() === "input") {
                        ...
                        if (i.attr("data-src")) val = i.attr("data-val");
                    } else if (i.hasClass("switch")) {
                        val = i.hasClass("selected") ? 1 : 0;
                    }
                    else if (i.hasClass("checkbox")) {
                        val = i.hasClass("btn-primary") ? i.attr("data-val") || 1 : 0;
                    } else if (i.hasClass("rdo")) {
                        val = $("[name='" + i.attr("name") + "'].radio.btn-primary").attr("data-val") || "";
                    }
                    else {
                        val = i.val();
                    }
                    if (!val && i.attr("data-val")) val = i.attr("data-val") || "";//单选
                    if (!d[n]) d[n] = "";
                    else d[n] += ",";
                    d[n] += val
                });
                if (def.m === 1) {
                    ...
                    if (def.postedcallback) {
                        try {
                            def.postedcallback(d);
                        } catch (e) { alert("callback 出错：" + e); }
                    }
                }
                else {
                    x.com.doapi(def.form.attr("api"), d, function (data) {
                        btn.removeAttr("disabled");
                        if (def.postedcallback && data.issucc) {
                            try {
                                if (def.m === 0) def.postedcallback(data);
                                else def.postedcallback(data, d);
                            } catch (e) { alert("callback 出错"); }
                        }
                    }, "json");
                }
            }
                ...
                btn.removeAttr("disabled");
                x.dialog.alert("有必填的项未填写，请填写好必填项（请注意标有红色框的项目）");
            }
            return false;
        });
        def.form.on("reset", function () {
            def.form.find("[data-src]").each(function () {
                $(this).attr("data-val", "").attr("parms", "").find(".text").text(($(this).attr("tip") || $(this).attr("placeholder")));
            });
            //def.form.submit();
        });
        checkinputs.each(function () {
            var input = $(this);
            if (input.data("evt")) { return; }
            input.bind({
                focus: function () {
                    var i = $(this);
                    def.hidemsg(i);
                    if (def.showtipicon && !i.attr("data-src")) {
                        var tip = i.data("tip");
                        if (!tip) {
                            tip = x.dialog({ id: "tip_" + i.attr("name"), autofocus: false, align: "right", content: i.attr("tip") || (i.attr("data-src") ? "请选择 " : "请输入 ") + (i.attr("placeholder") || i.attr("title") || i.text()) });
                            $(tip.node).addClass("ui-dialog-tip");
                            i.data("tip", tip);
                        }
                        tip.show(i[0]);
                    }
                ...
                keyup: function (key) {
                    var i = $(this);
                    if (!i.attr("data-check") || (key.keyCode < 20 && key.keyCode != 8)) return;
                    if (checkdata(i)) ispass += 0;
                    else ispass += 1;
                },
                blur: function () {
                    var i = $(this);
        ...
        function checkdata(i) {
            var o = {
                type: null,//类型：可为(mail|number|chinese|mphone|decimal|date|datetime)
                len: null,//长度：min,max
                rang: null,//范围：min,max
                re: null,//对照验证，例：{ name: null, type: 0 } name 对照选择器 type:-1:小于|0:等于|1:大于
                notempty: false//为空验证
            }
            try {
                o = $.extend(o, $.parseJSON(i.attr("data-check")));
            } catch (e) {
                console.log("验证Json有错：" + i.attr("data-check"));
            }
            var v = i.val() || i.attr("data-val");
            if (i.attr("data-src")) v = i.attr("data-val");
            else if (i.hasClass("checkbox")) {
                v = i.hasClass("btn-primary") ? i.attr("data-val") || 1 : 0;
            } else if (i.hasClass("rdo")) {
                v = $("[name='" + i.attr("name") + "'].radio.btn-primary").attr("data-val") || "";
            }

            var title = i.attr("placeholder");
            if (o.notempty && !v) {
                var msg = i.attr("tip") || (i.attr("data-src") || i.hasClass("checkbox") || i.hasClass("rdo") ? "请选择 " : "请输入 ") + (i.attr("placeholder") || i.attr("title") || i.text()).replace("请选择 ", "").replace("请输入 ", "")
                def.showmsg(i, msg);
                return false;
            }
            if (!v) { return true; }
            if (o.type) {
                if (validator[o.type]) {
                    if (o.type === "custom") {
                        ...
                        if (!reg.test(v)) {
                            def.showmsg(i, title + o.msg);
                            return false;
                        }
                    } else if (!validator[o.type].reg.test(v)) {
                        def.showmsg(i, title + validator[o.type].msg);
                        return false;
                    }
                }
            }
            if (o.re) {
                var r = o.re.split(",");
                var re = { obj: $("[name='" + r[0] + "']"), type: r[1] };
                var nv = re.obj.val();
                if (o.type === "number") {
                    nv = parseInt(re.obj.val());
                    v = parseInt(v);
                }
                else if (o.type === "ndecimal") {
                    nv = parseFloat(re.obj.val());
                    v = parseFloat(v);
                }
                if (re.type == -1 && v > nv) {
                    def.showmsg(i, title + " 的值要小于 " + re.obj.attr("placeholder") + " 的值。");
                    return false;
                } else if (re.type == 0 && v !== nv) {
                    def.showmsg(i, title + " 的内容与 " + re.obj.attr("placeholder") + " 的内容不一致。");
                    return false;
                } else if (re.type == 1 && v < nv) {
                    def.showmsg(i, title + " 的值要大于 " + re.obj.attr("placeholder") + " 的值。");
                    return false;
                }
            }
            if (o.len) {
                var l = o.len.split(",");
                if (l.length === 1 && v.length !== parseInt(l[0])) {
                    def.showmsg(i, title + " 的字符个数只能为 " + l[0] + " 个，现有 " + v.length + " 个。");
                    return false;
                }
                if (l.length === 2) {
                    if (l[0] === "" && l[1] === "") {
                        def.showmsg(i, title + " 的 len 参数设置错误，格式应为：min-max（min,max为 >0 的数）。");
                        return false;
                    }
                    if (l[0] === "") {
                        if (v.length > l[1]) {
                            def.showmsg(i, title + " 的字符个数要 <= " + l[1] + " 个，现有 " + v.length + " 个。");
                            return false;
                        }
                    } else if (l[1] === "") {
                        if (v.length < l[0]) {
                            def.showmsg(i, title + " 的字符个数要 >= " + l[0] + " 个，现有 " + v.length + " 个。");
                            return false;
                        }
                    } else if (v.length < l[0] || v.length > l[1]) {
                        def.showmsg(i, title + " 的字符个数只能在 " + o.len + " 个之间，现有 " + v.length + " 个。");
                        return false;
                    }
                }
            }
            if (o.rang) {
                ...
                if (l.length === 1 && v.length !== l[0]) {
                    def.showmsg(i, title + " 的值只能为 " + l[0] + "。");
                    return false;
                }
                if (l.length === 2) {
                    if (l[0] === "") {
                        if (v > ((o.type === "number" || o.type === "ndecimal") ? parseFloat(l[1]) : l[1])) {
                            def.showmsg(i, title + " 的值要 <= " + l[1] + "。");
                            return false;
                        }
                    }
                    else if (l[1] === "") {
                        if (v < ((o.type === "number" || o.type === "ndecimal") ? parseFloat(l[0]) : l[0])) {
                            def.showmsg(i, title + " 的值要 >= " + l[0] + "。");
                            return false;
                        }
                    } else if (v < ((o.type === "number" || o.type === "ndecimal") ? parseFloat(l[0]) : l[0]) || v > ((o.type === "number" || o.type === "ndecimal") ? parseFloat(l[1]) : l[1])) {
                        def.showmsg(i, title + " 的值只能在 " + o.rang + " 之间。");
                        return false;
                    }
                }
            }
...
//------------------------------------------------
// 字典选择器
// 0调用：模式:参数:联动:选择个数:按字母排
// 1模式：dict|view|div|date
// 2参数：dict时为字典编号；view时为地址；div时为id
// 3联动：目标name
// 4多选：选择个数
// 5回调：选择后的回调
//------------------------------------------------
x.dict = {
    ...
    init: function () {
        if ($("#dict_select").size() === 0) {
            $("body").append("<div id='dict_select' class='dict_select'><div class='h'></div><div class='c'></div><div class='b'><span class='btn btn-primary ok' onclick='x.dict.ok()'>确定</span> <span class='btn cancel' onclick='x.dict.hide()'>取消</span></div></div>");
        }
        $(document).click(function () {
            x.dict.hide();
        });
        this.def.div = $("#dict_select");
        this.def.div.h = $("#dict_select .h");
        this.def.div.c = $("#dict_select .c");
        this.def.div.b = $("#dict_select .b");
        this.def.div.bind({
            blur: function () { this.hide(); }
        });
        this.def.div.c.find(".dict-item").live("click", function (evt) {
            var item = $(this);
            if (!x.dict.def.selcount == 1) {
                $("#dict_select .dict-item.btn-primary").removeClass("btn-primary");
                item.addClass("btn-primary");
                x.dict.ok();
            } else {
                if (item.hasClass("btn-primary")) {
                    $(this).removeClass("btn-primary");
                    x.dict.def.div.h.text("最多能选" + x.dict.def.selcount + "个，已选" + $("#dict_select .dict-item.btn-primary").size() + "个");
                }
                else {
                    var sc = $("#dict_select .dict-item.btn-primary").size();
                    if (sc < x.dict.def.selcount) {
                        item.addClass("btn-primary");
                        x.dict.def.div.h.text("最多能选" + x.dict.def.selcount + "个，已选" + $("#dict_select .dict-item.btn-primary").size() + "个");
                    }
                }
    ...
    ok: function () {
        var vals = "";
        var texts = "";
        var i = x.dict.def.input;
        var datas = [];
        $("#dict_select .dict-item.btn-primary").each(function () {
            if (vals) { vals += ","; texts += "、"; }
            vals += $(this).attr("data-val");
            texts += $(this).text();
            datas.push(JSON.parse($(this).attr("data-data") || "{}"));
        });
        if (i[0].tagName.toLowerCase() !== "input") {
            i.find(".text").text((i.attr("tip") || i.attr("placeholder")) + "：" + texts).attr("txt", texts);
        }
        else {
            i.val(texts);
        }
        var d = i.attr("data-src").split(":");
        var to = $("[name='" + d[2] + "']");
        i.attr("data-val", vals);
        if (to) { to.attr("parms", "-" + vals).show().attr("data-val", "").find(".text").text(to.attr("tip") || to.attr("placeholder")); }
        this.hide();
        i.blur();
        if (d[5]) {
            eval(d[5] + "('" + texts + "', '" + vals + "',datas)");
        }
    },
    show: function (i) {
        if (!this.def.input || this.def.input[0] !== i[0]) {
            this.def.input = i;
            var d = i.attr("data-src").split(":");
            this.def.selcount = d[3];
            this.def.div.b[this.def.selcount > 1 ? "show" : "hide"]();
            this.loadcontent(d);
            if ((d[3] || "0") > 1) this.def.div.h.text("最多能选" + d[3] + "个").show();
            else this.def.div.h.hide().text("");
        }
        this.def.input = i;
        if (i[0].tagName == "INPUT") {
            ...
            i.keyup(function (event) {
                ...
                timer = setTimeout(function () {
                    x.dict.loadcontent(i.attr("data-src").split(":"));
                }, 500);
            });
        }
        this.def.div.css({
    ...
    loadcontent: function (d) {
        switch (d[0]) {
            case "dict":
                x.dict.sethtml("<span class='loading'>字典正在加载中...</span>");
                var url = "/com/dict-" + d[1] + (x.dict.def.input.attr("parms") || "-");
                if (d[4] != undefined) url += "-" + d[4];
                x.com.getview(url + ".html", function (data) {
                    x.dict.sethtml(data);
                    x.dict.loadback(d);
                });
                break;
            case "view":
                x.dict.sethtml("<span class='loading'>视图正在加载中...</span>");
                var u = "{key}-" + (x.dict.def.input.attr("parms") || "");
                u = d[1].replace("{p}", u);
                x.com.getview(u.replace("{key}", encodeURI(encodeURI(x.dict.def.input.val()))), function (data) {
                    x.dict.sethtml(data);
                    x.dict.loadback(d);
                });
                break;
            case "div":
                x.dict.sethtml($(d[1]).clone().html());
                x.dict.loadback(d);
                break;
        }
    },
    loadback: function (d) {
        var i = x.dict.def.input;
        if (i.attr("data-val")) {
            var v = i.attr("data-val").split(',');
            for (var i = 0; i < v.length; i++) {
                if (v[i]) $("#dict_select .dict-item[data-val='" + v[i] + "']").addClass("btn-primary");
            }
        }
    },
    sethtml: function (html) {
        this.def.div.c.html(html);
    },
    hide: function () {
        this.def.div.hide();
    }
...
x.com = {
    ...
    doapi: function (api, ps, callback, showtip) {
        ...
        if (showtip) x.dialog.loading();
        $.post("/api/" + api, ps, function (d) {
            if (showtip) x.com.closewin();
            if (!d.issucc) {
                x.dialog.alert(d.msg || d.errcode);
            }
            if (callback) {
                try {
                    callback(d);
     ...
     * callback：回调
    */
    doupload: function (file, type, callback) {
        var f = $(file)[0];
        var c = f.files.length;
        x.dialog.loading("正在上传...");
        for (var i = 0; i < c; i++) {
            if (f.files[i].size == 0) return;
            var form = new FormData();
            form.append("type", type);
            form.append("file", f.files[i]);// 文件对象
            var xhr = new XMLHttpRequest();
            xhr.onreadystatechange = function () {
                if (this.readyState != 4 || this.status != 200) return;
                if (--c == 0) x.com.closewin();
                var j = jQuery.parseJSON(this.responseText);
                if (j.issucc && callback) callback(j);
                else x.dialog.alert(j.msg);
            }
    ...
    openwin: function (url, title, closefun, op) {
        ...
        if (op.ismodal || true) win.showModal();
        else win.show();
    },
    /*
     * 关闭当前窗口
     */
    closewin: function () {
        var win = x.dialog.getCurrent();
        if (win) win.close().remove();
    },
    getview: function (url, callback) {
        if (x.com.ajax) x.com.ajax.abort();
        x.com.ajax = $.get(url, function (d) {
            x.com.ajax == null;
            if (callback) {
                callback(d);
            }
     ...
     * sobj：搜索值选择器
     */
    getbdpoint: function (obj, sobj, callback) {
        var n = $(sobj).val() || $(sobj).text() || $(sobj).attr("val");
        if (n) n = encodeURI(encodeURI(n));
        var point = $(obj).val();
        x.com.openwin(
            "/com/getpos-" + (n || "") + "-" + x.com.base64.encode(point) + ".html",
            "获取地图坐标",
            function (d) {
                if (d) {
                    var point = d.lng + "," + d.lat;
                    $(obj).val(point).text(point).attr("val", point);
                    if (callback) callback(d);
                }
            }, {
                w: $(window).width() * 0.75, h: $(window).height() * 0.7
            }
        );
    },
    showpoint: function (n, point) {
        if (n) n = encodeURI(encodeURI(n));
        x.com.openwin(
            "/com/getpos-" + (n || "") + "-" + x.com.base64.encode(point) + ".html", "查看地图"
        );
    },
    cookie: {
        ...
        get: function (name) {
            ...
            for (var i = 0; i < arrStr.length; i++) {
                ...
                if (temp[0] == name) return unescape(unescape(temp[1]));
            }
        },
        del: function (names) {
            var ns = names.split(",");
